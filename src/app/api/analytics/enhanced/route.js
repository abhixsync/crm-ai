...
export async function GET(request) {
  try {
    ...
    // Interested counts per campaign (N campaigns already limited to 10)
      ...
      campaigns.map((c) =>
        prisma.customer.count({
          where: {
            tenantId,
            status: "INTERESTED",
            campaignJobs: { some: { campaignId: c.id } },
          },
        })
      )
    );
    ...
    const campaignStats = campaigns.map((c, i) => {
      const counts = jobCountMap[c.id] || { total: 0, completed: 0, failed: 0 };
      const interestedForCampaign = interestedCounts[i] || 0;
