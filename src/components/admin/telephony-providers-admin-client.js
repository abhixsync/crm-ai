...
const PROVIDER_TYPES = ["TWILIO", "VONAGE", "PLIVO"];
...
export function TelephonyProvidersAdminClient({ initialProviders, embedded = false }) {
  ...
  async function fetchProviders(options = {}) {
    ...
    const response = await fetch("/api/admin/telephony-providers");
    const data = await response.json();

    if (!response.ok) {
      toast.error(data.error || "Unable to load telephony providers.");
      setLoading(false);
      return;
    }
  ...
  async function saveProvider(provider) {
    ...
    const response = await fetch(`/api/admin/telephony-providers/${provider.id}`, {
      method: "PATCH",
  ...
  async function setActiveProvider(providerId) {
    ...
    const response = await fetch(`/api/admin/telephony-providers/${providerId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
    ...
    toast.success(`Active telephony provider set to ${data.provider.name}.`);
    setSavingId("");
    await fetchProviders();
  }
  ...
  async function deleteProvider(providerId) {
    const confirmed = window.confirm("Delete this telephony provider configuration?");
    if (!confirmed) return;
  ...
  async function testProviderConnection(providerId) {
    ...
    const response = await fetch("/api/admin/telephony-providers/test-connection", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async function testAllProviderConnections() {
    ...
    if (!response.ok) {
      toast.error(data.error || "Failed to run telephony connectivity checks.");
    ...
    for (const result of data.results || []) {
      ...
      if (providerId) mapped[providerId] = result;
    }

    setConnectionResults((current) => ({ ...current, ...mapped }));
    setAllConnectionResults(data.results || []);
    toast.info(
      `Telephony checks completed. Success: ${data.successCount || 0}, Failed: ${data.failedCount || 0}.`
    );
    setTestingAllConnections(false);
  }
    ...
    <main className={embedded ? "space-y-4" : "mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 sm:py-8"}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Run live checks for all configured telephony providers in active/priority order.
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <Table>
              ...
              <TableBody>
                ...
                {!loading && orderedProviders.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={6}>No telephony providers configured.</TableCell>
                  </TableRow>
                ) : null}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-4">
          <div className="w-full max-w-4xl rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="grid gap-3 md:grid-cols-4">
              ...
              <Input
                placeholder="API Key / Secret payload"
